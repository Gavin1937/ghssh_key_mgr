...
def cli_create_new_gh_key(config, key_path):
  key_path = Path(key_path)
  if not key_path.exists() or not key_path.is_dir():
    raise ValueError(f'Input key_path doesn\'t exists or isn\'t a directory: {str(key_path)}')
  print('Create a new ssh key and upload to your github account...')
  status,resp = create_ghssh_key(key_path)
  if status_not_good(status):
    raise RuntimeError(f'Fail to create a new ssh key and upload to github.')
  print(f'Key Id: {resp["id"]}')
  print(f'Private Key: {resp["priv_key_path"]}')
  print(f'Public Key: {resp["pub_key_path"]}')
  ...
  try:
    if args.generate_config:
      ...
      if args.output and len(args.output) > 0:
        key_folder = args.output
      else:
        key_folder = Path('./keys').resolve()
        key_folder.mkdir(parents=True, exist_ok=True)
      cli_create_new_gh_key(config, key_folder)
    elif args.remove > 0:
